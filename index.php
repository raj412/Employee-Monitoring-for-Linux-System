  ...
  <?php
    ...
    if (get_gallery_content($galleryId, $folders, $images, $downloads) === FALSE) {
      ...
      foreach ($folders as $folder) {
        ...
        echo '<a class="folder-tile"'.$thumbNail.' href="?g='.rawurlencode($folder['link']).'"><span>'.ucfirst($folder['name']).'</span></a>';
      }
      ...
      for ($curImgIdx = ($curPage - 1) * MAX_IMAGES_PER_PAGE, $curImgNum = 0;
		...
		//$thumbNail = 'thumb.php?i='.urlencode($img['path']);
        $thumbNail = $img['path'];     
        $Img_name =  !empty($img['path'])? basename($img['path']): '';
...
?>
 ...
 echo '<p align= "right"> <a href="logout.php"><input name="Submit" type="submit" value="Logout" class="Button4"></a></p>';
  ?>
  </footer>
</div>
